...
parser.add_argument("--datadir", type=str, default="/root/Datasets/Cityscapes/",
                    help="directory the Cityscapes dataset is in")
parser.add_argument("--batch_size", type=int, default=8, help="batch size")
parser.add_argument("--num_gpu", type=int, default=2, help="number of gpus")
parser.add_argument("--losstype", type=str, default="segment",
                    help="choose between segment & reconstruction")
...
img_data = CityscapesDataset(
    args.datadir, split='train', mode='fine', augment=True)
img_batch = DataLoader(img_data, batch_size=args.batch_size,
...
generator = nn.DataParallel(UnetGenerator(3, img_data.num_classes, 64), device_ids=[
                            i for i in range(args.num_gpu)]).to(device)
...
for epoch in range(epochs):
    for idx_batch, (imagergb, labelmask, labelrgb) in enumerate(img_batch):
        ...
        if idx_batch % 1 == 0:
            ...
            v_utils.save_image(
                x.to(device).data, "./result/original_image_{}_{}.png".format(epoch, idx_batch))
            v_utils.save_image(
                labelrgb, "./result/label_image_{}_{}.png".format(epoch, idx_batch))
            ...
            if args.losstype == "segment":
                ...
                v_utils.save_image(
                    y_threshed, "./result/gen_image_{}_{}.png".format(epoch, idx_batch))
            else:
                v_utils.save_image(
                    y.data, "./result/gen_image_{}_{}.png".format(epoch, idx_batch))
